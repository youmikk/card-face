...
「提交图片」和「提交 logo」会把文件上传到审核服务（Cloudflare Worker，`review.youmikk.me`），先放进私有存储；只有审核通过后才会公开出现在对应分类里，未通过的内容不会公开。提交前必须在表单里勾选确认第七条（图片会上传，通过后会公开）。卡面可选纯色、银行、交通、其他；logo 沿用现有分类，角色为「银行」的分类还要指定银行编号。不要提交带卡号、姓名或真实银行卡照片的图片。
