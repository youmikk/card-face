...
「提交图片」和「提交 logo」把文件交给 Cloudflare Worker，先放进私有存储。审核通过后，刷新页面就会出现在对应分类。卡面可选纯色、银行、交通、其他；logo 沿用现有分类，银行 logo 还要指定银行。不要提交带卡号、姓名或真实银行卡照片的图片。

审核服务在 `intake/`。部署后如果地址不是 `app.js` 里的 `INTAKE`，只改那一行。审核密码只放在 Cloudflare 的环境变量里。
