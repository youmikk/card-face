...
A browser card designer. Upload a card, place payment logos, and export a PNG.
...
- A selected logo can be dragged, scaled, rotated, faded, locked, deleted, and reordered.
- In the layer list, click a name to select it and the button on the right to delete it. Press a name and drag to change the stacking order. The top row is the front layer.
- The reference image sits above the card and below the logos. It is for alignment only and is not included in the PNG.
- Rounded corners are on by default and apply to both the preview and the export.
- Chinese and English are available, along with light and dark themes. Language, theme, and the first-visit confirmation are stored locally.
- Terms of use appear on the first visit. The exported PNG is a visual design file only.
